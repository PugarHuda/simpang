...
export async function GET(req: Request) {
  ...
  const run = byId ? store.get(byId) : store.others(u.searchParams.get('exclude') ?? '')
  if (!run || (!byId && run.done)) return Response.json({ error: 'no other run in flight' }, { status: 404 })
